...
noise<-function(x, sd){x + rnorm(length(x),mean=0,sd)}

model_w_noise<-noise(CO2_predict,CO2_sigma)

#CO2_rnorm<-rnorm(length(CO2_predict),0,CO2_sigma)

#model_w_noise<-CO2_rnorm+CO2_predict
...
```{r, Section_3-2}
predictions<-map_df(0:1000, ~tibble(sim = noise(CO2_predict,CO2_sigma)))


# will plot obs CO2 vs pred CO2
predictions_GDP<-predictions|>
  mutate(obs_CO2=pred_obs_df$avg_CO2_emissions)
