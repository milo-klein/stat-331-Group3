...
The CO2 emissions data was taken from the Global Carbon Project, CDAIC and other sources. It documents the change in CO2 emissions produced by each country from 1800-2022. (https://docs.google.com/spreadsheets/d/1t_GOXGrXQgSasIB7dWfRrq9dkqjrqwy3xHvywH7ah1A/edit#gid=501532268). Originally, our CO2 emission data had 194 rows (Countries) and 224 columns (1800:2022). After pivoting longer, there were 43,262 rows (CO2 observations) and 3 columns (country, year, CO2).

For the GDP data, this was taken from multiple sources depending on the date range. From 1990 to 2019, data from the World Bank was primarily used, while from before 1990 was taken from Maddison Project Database and Penn World Table of the Groningen Growth and Development Centre. In summary, this data documents the change in GDP for most countries between years 1800 and 2100 (https://www.gapminder.org/data/geo/changes/). Furthermore, the data originally had 195 rows (Countries) and 302 columns (individual years, 1800:2100). After pivot: 58,695 rows (GDP observations) and 3 columns (country, year, GDP).

### **Data Cleaning Process and Cecisions**

After importing both data sets, we cleaned each by making sure all years were numeric and ready to merge. GDP values represented by characters such as “10k” were changed to numeric values. We also eliminated an “X” character from each of the years in the Emissions data set that prevented merging. Then, we pivoted both and conducted a full merge by “Year” and “Country”. Finally, we began filtering by years of interest. Future projections (years past 2021) were not considered in our data.
...
<center>y\^ = -2.863e-01 + (3.342e-04)\*x</center>

\n

The estimated slope coefficient for average GDP (code variable **`avg_GDP)`** is 3.342e-04, indicating that, on average, a one-unit increase in average GDP is associated with a Y-unit increase in average CO2 emissions. For example, a 1000 dollar increase in average GDP (variable name "avg_GDP"), on average, we expect that there would be 334.2 tons of CO2/capita.

The p-value for the average GDP (code name: **`avg_GDP)`** coefficient is less than 0.05, indicating that average GDP is a statistically significant predictor of average CO2 emissions.
...
The variance of the fitted values accounts for nearly 70.1% of variance response values while the residual variance accounts for 29.8%. This signifies that the linear regression model is a good fit for our observations, explaining that 70.1% of variability in average CO2 emissions is explained by the model (matching with the outputted R-squared value of 0.70).
...
Based on the plots shown above, the simulated data predicts fewer outliers at larger (above approximately \$15000) average GDP values. **Additionally, the two models differed in slope (** More specifically, the simulation predicted a larger slope compared to the data. Conversely, the two plots are similar in that both predict a positive-linear trend between average per capita CO2 emissions and GDP.
...
# Save animation
anim_save("CO2_vs_GDP.gif", animate(gif))
```
